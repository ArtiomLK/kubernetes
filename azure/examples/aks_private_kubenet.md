...
Azure Kubernetes Services private cluster architectural design with kubenet networking. Connect to an Azure Kubernetes Services private cluster by using Azure Private Links.
...
6. ### Create a Private Azure Kubernetes Service (AKS) with Kubenet
...
8. ### Create a new Application Gateway
...
9. ### Enable the AGIC add-on on an existing AKS cluster
   ...
   1. [Create a Private Azure Kubernetes Service (AKS) with Kubenet][106]
   ...
   # Disable addons if required
   # az aks disable-addons -n $aks_cluster_n -g $app_rg -a ingress-appgw
   # Enable AGIC on existing cluster
   az aks enable-addons -n $aks_cluster_n -g $app_rg -a ingress-appgw --appgw-id $AGIC_APPGW_ID

   # Associate the MC_$rg... route table to the Application Gateway sNet
   AKS_RT_ID=$(az network route-table list -g "MC_"$app_rg"_"$aks_cluster_n"_"$l  -o tsv --query "[0].id"); echo $AKS_RT_ID
   az network vnet subnet update \
   --vnet-name $vnet_n \
   --name $agic_snet_n \
   --resource-group $app_rg \
   --route-table $AKS_RT_ID
   ```

10. ### Create an Azure FrontDoor
...
13. ### Test Private Endpoint DNS Resolution
    ...
    # Uploads a Pod, a Service and a Ingress sample app
